...
# Тестирование API создания лидов (без jq)
# Автор: AI Assistant
# Дата: 2025-08-10
...
curl -s -X GET "$BASE_URL/api/leads/health"
echo ""
echo ""

# 2. Получение правил валидации
echo "2. Правила валидации (обязательные поля):"
curl -s -X GET "$BASE_URL/api/validation-rules" | grep -o '"required_fields":\[[^]]*\]'
echo ""
echo ""
...
curl -s -X POST "$BASE_URL/api/leads" \
  ...
  -d "email=test@example.com&name=Тест Пользователь&company=Тестовая Компания&phone=%2B7%20(900)%20123-45-67"
echo ""
echo ""
...
echo ""
echo "Для форматированного вывода установите jq: brew install jq"
